 ...
 * (modified by Becky Chao for personal use)
 *
 *
 */
...
window.formatGoogleCalendar = (() => {
    ...
    const transformationList = (result, tagName, format) => {
        ...
        for (j = 0; j < description.length; j++) {
            ...
            if (spaces < 46) {
                description2 = description2.concat(description[j]);
            }

            else {
                description3 = description3.concat(description[j]);
            }
        }

        description2 = description2.concat('...');    
        ...
        for (i = 0; i < format.length; i++) {
            ...
            if (format[i] === '*summary*') {
            ...
            } else if (format[i] === '*description2*') {
                output = output.concat(`<span class="description2">${description2}</span>`);
            } else if (format[i] === '*description3*') {
                output = output.concat(`<span class="description3">${description3}</span>`);    
                ...
                if ((format[i + 1] === '*location*' && location !== '') ||
                    ...
                    (format[i + 1] === '*description2*' && description2 !== '') || 
                    (format[i + 1] === '*description3*' && description3 !== '')) {
        ...
        return '\n\n' + '<div class="sidebar-box">' + output + '</' + tagName + '>' + '<p class="read-more"><a href="#" class="button">Read More</a></p></div>';
    };
    ...
    const formatDateSameDay = (dateStart, dateEnd, dayNames, moreDaysEvent, isAllDayEvent) => {
        ...
        if (config.sameDayTimes && !moreDaysEvent && !isAllDayEvent) {
            formattedTime = '<br><b>Time:</b> ' + getFormattedTime(dateStart) + ' - ' + getFormattedTime(dateEnd);
        }
    ...
    return { 
        init: function (settingsOverride) {
            var settings = {
                ...
                format: ['*date*', ': ', '*summary*', ' &mdash; ', '*description*', '*description2*', ' in ', '*location*'],
                timeMin: undefined,
                timeMax: undefined
            };
